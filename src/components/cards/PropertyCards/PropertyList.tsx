                    ...
                    <div className="p-3 md:p-4 lg:p-4 xl:p-6 2xl:p-6 flex flex-col flex-1 justify-between">
                        <div>
                            <div className="flex justify-between items-start gap-2">
                                <h3 className="text-sm md:text-xl font-semibold text-[#323232]">{property.propertyName}</h3>
                                <p className="text-sm md:text-xl font-bold text-[#7B4F3A]">
                                    ${property.propertyDetails?.price?.toLocaleString()}
                                </p>
                            </div>

                            <p className="text-[#646464] mt-0 md:mt-2 lg:mt-0 xl:mt-4 2xl:mt-4 text-xs md:text-base">{property.propertyDetails.location}</p>
                            ...
                            <div className="flex gap-2 md:gap-4 mt-2 md:mt-2 lg:mt-0 xl:mt-4 2xl:mt-4">
                                {property.propertyDetails?.bedrooms && (
                                    <div className="flex items-center gap-1">
                                        ...
                                        <span className="text-[#898989] text-xs md:text-base">{property.propertyDetails.bedrooms} Beds</span>
                                    </div>
                                )}
                                {property.propertyDetails?.bathrooms && (
                                    <div className="flex items-center gap-1">
                                        <FaBath className="text-[#898989] text-sm md:text-base" />
                                        <span className="text-[#898989] text-xs md:text-base">{property.propertyDetails.bathrooms} Baths</span>
                                    </div>
                                )}
                                {property.propertyDetails?.guests && (
                                    <div className="flex items-center gap-1">
                                        <IoPerson className="text-[#898989] text-sm md:text-base" />
                                        <span className="text-[#898989] text-xs md:text-base">{property.propertyDetails.guests} Guests</span>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="mt-2 md:mt-0 lg:mt-0 xl:mt-4 2xl:mt-4 flex flex-row justify-between items-center gap-2">
                            <span className="text-xs md:text-sm text-[#646464]">
                                Listed {property.listedDate ? new Date(property.listedDate).toLocaleDateString() : "Recently"}
                            </span>
                            <Link href={`/explore/${property.id}`}>
                                ...
                                <button className="underline text-[#7B4F3A] hover:bg-[#6A432F] text-xs block md:hidden">View Details</button>
                            </Link>
                        </div>
                    </div>
                </div>
