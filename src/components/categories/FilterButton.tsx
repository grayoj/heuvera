...
import { LucideSlidersHorizontal, X, Bed, Home, Bath, Wifi, Utensils, Shield, Refrigerator, SlidersHorizontal } from "lucide-react"
...
export function FilterModal() {
 ...
 const [priceRange, setPriceRange] = useState<[number, number]>([45000, 105000])
 const [bedrooms, setBedrooms] = useState<string | null>(null)
 const [beds, setBeds] = useState<string | null>(null)
 const [bathrooms, setBathrooms] = useState<string | null>(null)
 const [amenities, setAmenities] = useState<string[]>(["Wifi", "Kitchen", "Washer"])
 ...
 const handleNumberSelect = (type: "bedrooms" | "beds" | "bathrooms", value: string) => {
 switch (type) {
 case "bedrooms":
 ...
 className={cn(
 "flex items-center justify-center gap-1 py-4 rounded-none",
 activeTab === "amenities" ? "border-b-2 border-[#8B4513]" : "",
 )}
 >
 ...
 bedrooms === num ? "bg-white ring-1 ring-[#8B4513]" : "bg-[#f0efe9]",
 )}
 ...
 <div className="flex gap-2 flex-wrap">
 {["Any", "1", "2", "3", "4", "5", "5+", "6"].map((num) => (
 <button
 key={`bed-${num}`}
 className={cn(
 "rounded-full py-2 px-4 border",
 beds === num ? "bg-white ring-1 ring-[#8B4513]" : "bg-[#f0efe9]",
 )}
 ...
 className={cn(
 "rounded-full py-2 px-4 border",
 bathrooms === num ? "bg-white ring-1 ring-[#8B4513]" : "bg-[#f0efe9]",
 )}
 ...
 amenities.includes("Wifi") ? "bg-[#f8efe9] border-[#8B4513]" : "bg-[#f0efe9]",
 )}
 onClick={() => toggleAmenity("Wifi")}
 >
 <Wifi className="h-5 w-5" />
 <span>Wifi</span>
 {amenities.includes("Wifi") && <span className="ml-auto">✓</span>}
 </button>

 <button
 className={cn(
 "flex items-center gap-2 rounded-full py-3 px-4 border",
 amenities.includes("Washer") ? "bg-[#f8efe9] border-[#8B4513]" : "bg-[#f0efe9]",
 )}
 onClick={() => toggleAmenity("Washer")}
 >
 <Bath className="h-5 w-5" />
 <span>Washer</span>
 {amenities.includes("Washer") && <span className="ml-auto">✓</span>}
 </button>

 <button
 className={cn(
 "flex items-center gap-2 rounded-full py-3 px-4 border",
 amenities.includes("Kitchen") ? "bg-[#f8efe9] border-[#8B4513]" : "bg-[#f0efe9]",
 )}
 onClick={() => toggleAmenity("Kitchen")}
 >
 <Utensils className="h-5 w-5" />
 <span>Kitchen</span>
 {amenities.includes("Kitchen") && <span className="ml-auto">✓</span>}
 </button>

 <button
 className={cn(
 "flex items-center gap-2 rounded-full py-3 px-4 border",
 amenities.includes("Security cameras") ? "bg-[#f8efe9] border-[#8B4513]" : "bg-[#f0efe9]",
 )}
 onClick={() => toggleAmenity("Security cameras")}
 >
 <Shield className="h-5 w-5" />
 <span>Security cameras</span>
 {amenities.includes("Security cameras") && <span className="ml-auto">✓</span>}
 </button>

 <button
 className={cn(
 "flex items-center gap-2 rounded-full py-3 px-4 border",
 amenities.includes("Refrigerator") ? "bg-[#f8efe9] border-[#8B4513]" : "bg-[#f0efe9]",
 )}
 onClick={() => toggleAmenity("Refrigerator")}
 >
 <Refrigerator className="h-5 w-5" />
 <span>Refrigerator</span>
 {amenities.includes("Refrigerator") && <span className="ml-auto">✓</span>}
 </button>
 </div>
 ...
 <Button className="rounded-full bg-[#8B4513] hover:bg-[#7a3b10] text-white">Find 33 results</Button>
 </div>
 </div>
 </div>
 </>
