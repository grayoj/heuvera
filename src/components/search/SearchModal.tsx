...
import { LucideX, LucideSearch, LucideMapPin, LucideCalendar, LucideUsers } from "lucide-react";
import { SearchModalProps } from "@heuvera/utils/props";

const SearchModal: React.FC<SearchModalProps> = ({ isOpen, closeModal, activeFilter, toggleFilter, inputRef }) => {
    ...
    return (
        <AnimatePresence>
            ...
            >
                ...
                >
                    <button onClick={closeModal} className="p-2 rounded-full hover:bg-[#F8F7F2] dark:bg-[#333333]">
                ...
                >
                    <div className="px-2 pt-2 flex gap-2 overflow-x-auto no-scrollbar rounded-t-xl bg-[#F8F7F2] dark:bg-[#333333]">
                        ...
                        { id: "guests", label: "Who", icon: <LucideUsers size={16} /> }].map((filter) => (
                            <motion.button
                                key={filter.id}
